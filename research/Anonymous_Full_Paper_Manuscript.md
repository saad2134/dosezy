...
| Dimension | Commercial mHealth Apps | Smart Electronic Pillboxes | Proposed System |
| :--- | :--- | :--- | :--- |
...
We developed the application in Kotlin using Jetpack Compose, targeting Android 8.0 (API level 26) through Android 14 (API level 34) [9]. The codebase follows Clean Architecture principles, establishing strict separation between presentation composables, domain business logic, and local database persistence.
...
Smartphone operating systems aggressively optimize battery life by restricting background processes when the screen is turned off. To guarantee that medication alarms fire reliably during deep sleep, the application implements a multi-stage background scheduling pipeline:
