...
Prescription non-adherence among older adults poses substantial health risks and generates preventable healthcare expenditures. Although mobile medication reminder applications are widely available, existing commercial and open-source tools exhibit fundamental human-computer interaction (HCI) and background scheduling limitations when deployed in geriatric contexts: they rely on circular radial clock dials that require fine continuous motor tracing, enforce mandatory cloud registration that aggregates sensitive medical profiles, and suffer from dropped notifications caused by aggressive Android OEM background battery optimization. While individual foundational primitives—such as Fitts' law target sizing, relational SQLite storage, `AlarmManager.setAlarmClock` scheduling, and standardized usability scales—are well established, their unified integration into a cohesive, geriatric-accessible, and offline-contained mobile architecture has not been previously investigated in medication reminder systems. We present **Dosezy**, an open-source, local-first Android medication reminder system engineered specifically to address these physical, cognitive, and operating system barriers. Dosezy introduces a stationary 12-Hour Grid Time Picker featuring a 12-first layout and oversized $\ge 56\,\text{dp}$ touch targets, enforces architectural network isolation via manifest privilege minimization through the total omission of the `android.permission.INTERNET` permission, generates patient medication and adherence summary PDFs entirely on-device, and implements a multi-stage background scheduling pipeline combining `setAlarmClock` and exact alarm APIs with full-screen lock screen intent handling. We report empirical evaluations across systems and human factors dimensions: (1) multi-device runtime profiling on flagship and entry-level hardware demonstrating cold startups under $780\,\text{ms}$, vector PDF generation in $142 \pm 18\,\text{ms}$, and database query latencies under $12\,\text{ms}$, establishing engineering feasibility against Android 60 fps (16.6 ms) frame budgets; (2) a multi-OEM Doze mode alarm benchmark across five physical devices (99 of 100 scheduled alerts met the prespecified $\le 5.0\,\text{s}$ criterion under tested deep Doze and device conditions, $99.0\%$, 95% CI $[94.55\%, 99.82\%]$), confirming that application-level techniques cannot completely eliminate OEM-specific power-management interference without user-side whitelisting; and (3) an initial counterbalanced within-subjects laboratory usability study ($N=16$ older adults, aged 61--76, mean age $68.5 \pm 4.8$ years) demonstrating a $44.4\%$ reduction in total time-setting latency ($19.91\,\text{s}$ vs. $35.79\,\text{s}$, $p < 0.001$), an $80.0\%$ reduction in touch input errors ($0.44$ vs. $2.19$, $p < 0.001$), and a System Usability Scale (SUS) score of $83.44 \pm 9.48$ (corresponding to Grade A, 'Excellent' under the normative framework of Bangor et al. [14, 15]) compared to $57.19 \pm 12.45$ for standard radial dials. We frame our contributions as systems and usability engineering prerequisites for medication adherence, discussing threats to validity and longitudinal trade-offs.
...
Addressing these research questions motivated the design, implementation, and empirical evaluation of **Dosezy**, an open-source, local-first Android medication reminder platform. While foundational interaction and systems primitives—including Fitts' law target scaling, relational SQLite storage, `AlarmManager.setAlarmClock` scheduling, and standardized usability metrics—are established, to the best of our knowledge, prior work has not empirically evaluated this specific systems integration of discrete stationary time input, local-only data containment, and Doze-oriented alarm scheduling in a digital medication reminder system designed specifically for older adults. Our research makes four primary contributions:

- **Integrated Accessibility-Centric Interaction Architecture:** We propose and validate the *12-Hour Grid Time Picker*, an interface replacing circular dragging gestures with a 12-first discrete button grid ($56\times56\,\text{dp}$ touch targets) and quick-selection minute chips (:00, :15, :30, :45) that enable time configuration in as few as two stationary taps, paired with a visual palette whose primary text/surface pair achieves a measured contrast ratio of $7.84:1$, exceeding the WCAG 2.1 AAA minimum threshold ($7.0:1$) for normal text [17].
- **Architectural Network Isolation via Privilege Minimization:** We present a local-first systems architecture that enforces on-device data containment by completely omitting `android.permission.INTERNET` from the manifest. This establishes an operating-system-level socket denial boundary that prevents ordinary application-level network access through Android's permission-controlled socket APIs, coupled with an on-device vector A4 PDF export engine ($142 \pm 18\,\text{ms}$) that executes without remote cloud dependencies [16].
- **Doze-Resilient Alarm Scheduling Pipeline:** We engineer a multi-stage background alarm dispatch pipeline combining `AlarmManager.setAlarmClock`, exact alarm privileges, and full-screen lock screen intents to maximize timely dosage delivery during deep Doze states across fragmented Android OEM distributions, while characterizing the OS-level boundaries of vendor-specific background optimization.
- **Empirical Systems and Usability Validation:** We conduct hardware runtime profiling establishing sub-frame execution feasibility, benchmark background alarm delivery across five smartphone OEMs under deep Doze mode ($99.0\%$ on-time delivery across 100 test alarms under tested conditions), and execute an initial counterbalanced within-subjects laboratory usability study ($N=16$ older adults, aged 61–76) demonstrating statistically significant reductions in task completion time ($44.4\%$) and input errors ($80.0\%$), yielding a mean System Usability Scale (SUS) score of $83.44 \pm 9.48$, corresponding to Grade A ('Excellent') under the normative framework of Bangor et al. [14, 15] compared to $57.19 \pm 12.45$ for standard radial dials.
...
3. **Commercial Mobile Applications:** Commercial mHealth apps (e.g., Medisafe, MyTherapy) operate on commodity hardware already carried by patients. However, their systems architecture typically introduces friction for older adults. As summarized in Table 1, commercial applications frequently mandate remote account registration, integrate commercial advertisement banners and third-party telemetry SDKs, require navigating multi-screen onboarding wizards, and rely on standard cloud push notifications or inexact alarm primitives that are susceptible to deferral or termination when modern OEM power-management daemons force deep sleep [11, 23].

**Dosezy's Architectural Synthesis:** Dosezy addresses these systemic limitations by presenting an integrated, open-source mobile architecture that unifies an accessible 12-first stationary touch grid with Doze-resilient scheduling and zero network footprint. By operating 100% on-device, Dosezy eliminates financial barriers, protects medical confidentiality, and achieves high-reliability alarm dispatch without cloud or hardware dependencies.
...
**Research Gap and Synthesis:** Prior literature has investigated geriatric touchscreen motor challenges [9, 10], local-first data systems [16], and mobile operating system background scheduling [23] primarily within separate disciplinary silos. Human factors research rarely addresses the low-level operating system scheduling mechanisms necessary to ensure that reminder notifications wake dormant hardware on fragmented OEM distributions. Conversely, mobile systems engineering work often focuses on battery conservation benchmarks while taking interface accessibility for granted. To the best of our knowledge, Dosezy is the first mobile health system that empirically investigates the intersection of stationary discrete time input, on-device data containment, and Doze-resilient background scheduling targeted specifically at the operational needs of older adults.
...
**Threat Model and Security Boundaries:** We explicitly define the threat model and protection boundaries established by this architecture:
- *In-Scope Threats Addressed:* The omission of `android.permission.INTERNET` defends against unauthorized remote data exfiltration, covert analytics beaconing, remote telemetry harvesting, and inadvertent network transmission of sensitive patient medication schedules by embedded libraries. Even if a third-party dependency executes malicious routines within Dosezy's process, the Linux kernel and Android zygote enforce an unbypassable socket creation block at the cgroup/eBPF boundary.
- *Out-of-Scope / Environmental Threats:* We explicitly delineate this boundary from full-device cryptographic isolation. This architectural design does not claim device-level encryption at rest (which depends on host Android full-disk encryption and keystore capabilities), immunity to local Inter-Process Communication (IPC) compromises by other privileged applications, or protection against physical device compromise or rooted firmware. Local document exports via `FileProvider` content URIs are intentionally mediated by the Android system share sheet, requiring deliberate, user-confirmed action to transfer generated PDF summaries or backup archives to external applications.
...
1. **Primary Tier (Modal Interval Selection):** Oversized stationary discrete targets for common quarter-hour prescription intervals (e.g., :00, :15, :30, :45) aligned with standard outpatient dosing routines, bounding motor complexity to 2–3 stationary taps.
...
4. **Preset Minute Quick-Chips with Fine Adjustment Stepper:** Outpatient pharmacotherapy regimens are predominantly scheduled at standard hour and quarter-hour milestones tied to daily physiological events (e.g., waking at 8:00 AM, post-lunch at 1:30 PM, bedtime at 9:00 PM) [22]. Dosezy provides four discrete quick-select chips: `:00`, `:15`, `:30`, and `:45`. Because the interface pre-selects the `:00` minute chip and defaults to the current diurnal period (e.g., AM for morning sessions), scheduling an on-the-hour dose requires exactly two taps:
...
This measured value exceeds the $7.0:1$ contrast ratio mandated by WCAG 2.1 Level AAA for normal text [17], specifically ensuring that the primary typography and key visual containers maintain high contrast under suboptimal viewing angles and degraded retinal illumination.
...
$$\text{SR-ICR} = \left(\frac{\sum_{d=1}^{W} \mathbb{I}_d}{W}\right) \times 100\%$$

where $W$ is the observation window (typically 30 or 90 days), and $\mathbb{I}_d$ is the binary intake indicator returning 1 if all scheduled medications for calendar day $d$ were marked `TAKEN`, and 0 otherwise. We explicitly delineate SR-ICR from both pharmacy claims-based PDC and direct clinical adherence: while claims-based PDC measures prescription refill velocity across administrative pharmacy claims, and clinical adherence reflects biological drug ingestion, SR-ICR quantifies an on-device application-level intake-confirmation index. It captures patient engagement and recording fidelity within the software rather than biological therapeutic efficacy. SR-ICR is defined as an application-level logging index for future longitudinal trials; no claims about pharmacological or clinical medication adherence are made from it in the present study.
...
```text
Algorithm 1: Alarm Scheduling and Doze-Resilient Dispatch Pipeline
---------------------------------------------------------------------------------
...
1. **System Alarm Clock Elevation (`setAlarmClock`):** Rather than using standard idle-tolerant alarms, Dosezy registers reminders via `AlarmManager.setAlarmClock(AlarmClockInfo, PendingIntent)`. The Android operating system treats `setAlarmClock` as a high-priority alarm type under the Android Open Source Project (AOSP), originally reserved for user-facing clock utilities. Alarms scheduled through this method:
   - Cause the operating system to exit low-power idle modes (Doze) to deliver the alarm broadcast precisely at the scheduled real-time clock (`RTC_WAKEUP`) tick.
   - Cause the system status bar to display the official platform alarm clock icon, providing visual confirmation that an alert is armed.
   - Are exempted from standard background power throttling, although they do not bypass Android 10+ background activity launch restrictions—which is precisely why Dosezy pairs alarm reception with full-screen notification intents.
2. **Android 12–14 Permission Negotiation and Distribution Realism:** Modern Android versions impose nuanced permission models that directly impact medication scheduling reliability:
   ...
   For distribution targeting API 33+, Dosezy declares `USE_EXACT_ALARM` under the core alarm exemption, with fallback to `SCHEDULE_EXACT_ALARM` qualified for API 31–32 via `android:maxSdkVersion="32"` or runtime status checking via `AlarmManager.canScheduleExactAlarms()`. At runtime, the application evaluates `AlarmManager.canScheduleExactAlarms()`. If exact scheduling is revoked or denied (such as on fresh Android 14 installations where users or OEM policies have revoked exact alarm capability), Dosezy presents a high-contrast explanation dialog and dispatches an explicit system intent:
...
3. **Full-Screen Lock Screen Activity Overlay via Compliant Intents:** Under Android 10+ Background Activity Launch rules, background receivers cannot directly call `startActivity()`. Dosezy conforms by dispatching a high-priority heads-up notification with `setFullScreenIntent(pendingIntent, true)`. The target `AlarmActivity` configures its window via API 27+ methods `setShowWhenLocked(true)` and `setTurnScreenOn(true)`, paired with `FLAG_KEEP_SCREEN_ON`. Starting in Android 14 (API 34), Google restricted the automatic grant of `android.permission.USE_FULL_SCREEN_INTENT` to applications primarily categorized as calling or alarm utilities on Google Play [25]. Applications submitting to commercial stores outside these categories have the permission revoked by default. Dosezy handles this operating system boundary through runtime verification: on API 34+, the application evaluates `NotificationManager.canUseFullScreenIntent()`. If granted (as is standard in alarm-category listings and open-source distributions such as F-Droid), the system immediately illuminates the screen and presents the full-screen dosage prompt over the keyguard. If revoked, the notification gracefully falls back to an expanded heads-up banner in the active notification shade with audible and tactile alerting, while prompting the user to grant full-screen access via `Settings.ACTION_MANAGE_APP_USE_FULL_SCREEN_INTENT`. When active, the overlay enables older patients to visually review the medication name, dosage strength, and instructions, tapping "TAKEN" or "SNOOZE 10M" with a single oversized tap without navigating lock screen PINs, patterns, or biometric sensors.
...
*Note: The offline emergency dispatch resolution table is provided as an immediate convenience feature and is not a substitute for authoritative local emergency-service information.*
...
#### Table 3: Hardware Performance and Micro-Benchmark Profiling (Mean ± SD across 10 trials)
...
As documented in Table 3, Dosezy achieved cold startup times well under 1 second even on the low-tier Exynos 850 processor ($780 \pm 45\,\text{ms}$), while maintaining a steady-state heap allocation of approximately $35\,\text{MB}$. The combined startup standard deviation of $185\,\text{ms}$ across both devices represents the aggregate sample standard deviation across all 20 observations from two hardware tiers with distinct baseline performance, rather than a pooled within-device standard deviation. Retrieving 500 historical dosage logs from the Room SQLite database completed in $8.8 \pm 3.1\,\text{ms}$ on average; while this benchmark falls within a 16.6 ms (60 Hz) display frame budget, we emphasize that in production, Room executes all database queries asynchronously off the UI thread via Kotlin coroutines on `Dispatchers.IO`. On-device vector PDF report generation required $142 \pm 18\,\text{ms}$ in a background worker, and ISO emergency number resolution executed in under $1\,\text{ms}$. Rather than asserting comparative superiority over commercial cloud applications whose runtime footprints depend heavily on network conditions, these micro-benchmarks establish engineering feasibility against Android memory quotas and execution budgets on budget hardware.
...
On each of the five devices, we scheduled 20 discrete medication alarms spaced at 15-minute intervals (100 total alarms clustered across 5 physical devices). To ensure rigorous experimental control: (1) each device was positioned stationary on a flat surface; (2) USB cables were disconnected immediately prior to trial commencement (with ADB executed over local TCP/IP or detached before idle entry); (3) battery levels were maintained between 50% and 80%; (4) displays were powered off; and (5) the device was forced into deep Doze mode via ADB:
`adb shell dumpsys deviceidle force-idle`
Successful entry into deep idle was verified via `adb shell dumpsys deviceidle get deep` returning `IDLE`.
...
An alarm was operationalized as **ON-TIME** if $\Delta t_{\text{latency}} \le 5.0\,\text{seconds}$. We operationalized on-time delivery as $\le 5.0\,\text{seconds}$ because medication reminders are immediate, user-facing critical alerts where a perceptible delay is disruptive, whereas general background data synchronization tolerances can extend over minutes or hours.

#### Table 4: Multi-OEM Alarm Delivery Benchmark Across Deep Doze States (20 Test Alarms per Device)

| Device Model | OS Distribution | On-Time Delivery | Mean Latency (s) | Max Latency (s) | Success Rate (95% CI) |
| :--- | :--- | :---: | :---: | :---: | :---: |
| Google Pixel 7 | Android 14 (Stock) | 20 / 20 | $1.55 \pm 0.25$ | 2.0 | 100.0% |
| Samsung Galaxy S22 | One UI 5.1 (API 33) | 20 / 20 | $2.15 \pm 0.24$ | 2.5 | 100.0% |
| Xiaomi Redmi Note 8T | MIUI 14 (API 33) | 19 / 20 | $4.87 \pm 14.96^*$ | $68.4^*$ | 95.0% |
| OnePlus 10 Pro | OxygenOS 13 (API 33) | 20 / 20 | $1.85 \pm 0.23$ | 2.2 | 100.0% |
| Motorola Moto G Power | Moto My UX (API 32) | 20 / 20 | $2.45 \pm 0.30$ | 2.9 | 100.0% |
| **Total / Aggregate** | --- | **99 / 100** | **2.57 ± 6.66 (incl.); 1.91 ± 0.44 (excl.)** | **68.4 (incl.); 2.9 (excl.)** | **99.0% [94.55%, 99.82%]** |

*Note on Outlier Dispersion:* Across the 19 on-time alarms on the Xiaomi device, mean latency was $1.52 \pm 0.28\,\text{s}$ (max $2.0\,\text{s}$). Including the single $68.4\,\text{s}$ OEM powerkeeper delay, sample standard deviation expands to $14.96\,\text{s}$ (mean $4.87\,\text{s}$). Aggregate success proportion reports the exact Wilson score 95% binomial confidence interval ($[94.55\%, 99.82\%]$).

*Benchmark Delivery Analysis and OEM Throttling Observations:* In our five-device, 100-alarm benchmark under tested deep Doze conditions, 99 of 100 alarms ($99.0\%$, 95% CI $[94.55\%, 99.82\%]$) triggered within 5 seconds (Table 4). Crucially, this empirical benchmark illustrates that application-space design cannot completely eliminate vendor-level battery killer interference across all hardware without explicit user whitelisting. The sole observed delay occurred on the Xiaomi Redmi Note 8T running MIUI 14, where a single alarm was held back for $68.4\,\text{seconds}$ until the screen was illuminated. Telemetry inspection confirmed that Xiaomi's proprietary powerkeeper daemon (`com.miui.powerkeeper`) temporarily suspended broadcast execution despite the elevated `setAlarmClock` status. A similar delay profile occurs on Samsung One UI if an app is categorized under "Deep Sleeping Apps". While `setAlarmClock` provides the highest execution priority granted by AOSP, vendor-specific battery daemons remain an operating system barrier that cannot be unilaterally bypassed without user-side battery exemption. When users manually exempt Dosezy from background restrictions via system settings, delivery across test trials reached 100% ($20/20$).
...
The final cohort comprised 16 older adults ($N=16$, aged 61 to 76 years, mean age $68.5 \pm 4.8$ years, median $68.5$; 9 female, 7 male). All participants were active prescription drug consumers managing two or more daily medications (mean $3.8 \pm 1.2$ daily drugs). Twelve participants ($75.0\%$) used prescription reading glasses, and four participants ($25.0\%$) exhibited mild physiological resting or action hand tremors. Table 5 details the complete participant demographic profile.

#### Table 5: Usability Study Participant Demographics and Clinical Characteristics (N=16)

| Demographic / Characteristic | Cohort Value (N=16) | Measurement Notes |
| :--- | :---: | :--- |
| **Age (Years)** | $68.5 \pm 4.8$ | Range: 61–76 years (Median: 68.5) |
| **Gender** | 9 Female ($56.2\%$), 7 Male ($43.8\%$) | Self-reported |
| **Prescription Reading Glasses** | 12 ($75.0\%$) | Corrected vision $\ge 20/60$ confirmed |
| **Mild Physiological Hand Tremor** | 4 ($25.0\%$) | Clinically observed resting/action tremor |
| **Daily Prescription Regimen** | $\ge 2$ medications/day | Inclusion criteria; cohort mean $3.8 \pm 1.2$ drugs |
| **MMSE Cognitive Score** | $28.6 \pm 1.1$ | Mini-Mental State Exam ($\ge 27/30$ cutoff) |
| **Counterbalance Sequence** | 8 Grid-First (AB), 8 Radial-First (BA) | Randomly assigned 1:1 allocation |

**Exploratory Subgroup Observations (Hand Tremor):** Four participants presented with mild hand tremors. Descriptively, these four participants averaged $19.35\,\text{s}$ under the 12-Hour Grid vs. $36.82\,\text{s}$ under the radial dial, with touch errors dropping from $2.25$ to $1.00$. In the non-tremor cohort ($n=12$), mean times were $20.10\,\text{s}$ vs. $35.45\,\text{s}$, and errors dropped from $2.17$ to $0.25$. Because $n=4$ lacks statistical power for formal inferential subgroup testing, we report these patterns strictly as exploratory observations. We also explicitly note that our exclusion of severe visual impairments and Parkinsonian stage IV/V tremors means our empirical findings characterize older adults with relatively preserved functional dexterity, rather than advanced neurodegenerative motor deficits.

**A Priori Power Analysis:** An a priori statistical power analysis was conducted using G*Power 3.1 for a paired within-subjects Student's $t$-test (two-tailed $\alpha = 0.05$, desired power $1 - \beta = 0.80$, assumed effect size $d_z = 0.80$ based on prior mobile accessibility literature for older adults [9, 10]), yielding a minimum required sample of $N = 15$. The enrolled sample of $N = 16$ participants satisfies this prerequisite.
...
**Timing Instrumentation & Algorithmic Error Operationalization:** Interaction timestamps were captured automatically with millisecond precision via Jetpack Compose `pointerInput` event listeners attached to the root dialog surface. Touch input errors ($TIE$) were operationalized as algorithmically detected touch-error events through deterministic coordinate logging logic:
- *Target Slip (Grid & Radial):* Recorded when pointer-down coordinates fell within target bounding box $B_i$, but pointer-up coordinates fell outside $B_i$ or crossed gutter boundaries prior to release ($\text{target}_{\text{down}} \ne \text{target}_{\text{up}}$).
- *Immediate Reselection (Grid):* Recorded when an hour or minute chip was selected and subsequently replaced by an adjacent target within $1.5\,\text{seconds}$ without an intervening confirmation tap. The 1.5-second threshold operationalizes Fitts' hesitation and recovery model, capturing instances where an older adult corrects an unintended adjacent tap without executing an intermediate action.
- *Quadrant Steering Abort (Radial):* Recorded when a continuous circular drag in Condition B traversed through an incorrect angular quadrant and was released at an angular deviation $|\theta_{\text{release}} - \theta_{\text{target}}| > 15^\circ$, necessitating an immediate secondary correction drag. The 15° threshold corresponds to half the angular width of an hour sector ($360^\circ / 12 = 30^\circ$), capturing instances where continuous steering crossed the midpoint into an adjacent sector, requiring trajectory realignment.

All error events were logged algorithmically from raw coordinate streams rather than subjective human video coding, eliminating observer scoring bias. We acknowledge that while algorithmic coordinate parsing eliminates observer subjectivity, automated thresholds cannot completely distinguish exploratory hesitation from involuntary motor slips.
...
- **Condition B (Radial Dial Mode Baseline):** Setting reminder times using the official Material Design 3 Jetpack Compose `TimePicker` (`androidx.compose.material3:material3:1.2.0`, vertical layout `TimePickerLayoutType.Vertical`) running in 12-hour radial dial mode. To isolate pure continuous circular motor trajectory steering friction, the alternative soft-keyboard text input toggle was disabled. We explicitly acknowledge this methodological boundary: while standard Android deployments permit toggling between radial dragging and text entry, disabling the toggle isolated curved motor steering mechanics from numeric soft-keyboard typing, while framing keyboard input and other discrete pickers as important targets for future comparative evaluation.

Participants were randomly allocated to two counterbalanced ordering cohorts: Cohort 1 ($n=8$) completed Condition A followed by Condition B; Cohort 2 ($n=8$) completed Condition B followed by Condition A. To evaluate potential carryover effects, we computed within-subject difference scores ($D_i = \text{Score}_{A,i} - \text{Score}_{B,i}$) and compared them across sequence cohorts ($AB$ vs. $BA$). Independent-samples $t$-tests detected no statistically significant sequence effect on task completion time ($t(14) = 0.38, p = 0.71$) or SUS scores ($t(14) = 1.18, p = 0.26$); however, we explicitly emphasize that with $n=8$ per sequence cohort, statistical power to formally establish equivalence is limited.

#### 6.3.3 Task Protocol and Interaction Sequences
Participants completed three prescription scheduling tasks selected to evaluate progressive motor demands across outpatient pharmacotherapy patterns [22]:
- **Task 1 (Modal On-the-Hour):** Configure a reminder for **8:00 AM** (standard morning prescription, default :00 minute chip).
- **Task 2 (Modal Half-Hour Preset):** Configure a reminder for **1:30 PM** (post-lunch prescription requiring AM/PM toggle and selecting :30 minute chip).
...
Table 6 contrasts the concrete gesture sequences required under both interfaces across each task.

#### Table 6: Comparative Interaction Gesture Sequences: 12-Hour Grid vs. Standard Radial Dial

| Task / Target | Condition A (12-Hour Grid Picker) | Condition B (Standard Radial Clock) |
| :--- | :--- | :--- |
| **Task 1 (8:00 AM)** | Tap `8` (AM and :00 default) → Tap `OK` [2 discrete stationary taps] | Touch pin → drag hand along 240° arc to `8` → release → drag hand to `00` → release → Tap `OK` [2 continuous curved drags + 1 tap] |
| **Task 2 (1:30 PM)** | Tap `PM` → Tap `1` → Tap `:30` chip → Tap `OK` [4 discrete stationary taps] | Tap `PM` → drag hand along arc to `1` → release → drag hand along 180° arc to `30` → release → Tap `OK` [2 continuous curved drags + 2 taps] |
| **Task 3 (9:42 PM)** | Tap `PM` → Tap `9` → Tap `:45` chip → Tap `[-]` stepper × 3 → Tap `OK` [7 discrete stationary taps] | Tap `PM` → drag hand to `9` → release → drag hand along 252° arc to exact minute `42` → precision release → Tap `OK` [2 continuous curved drags + fine release + 2 taps] |
...
#### Table 7: Comparative Usability Study Findings (N=16, Paired Within-Subjects Evaluation)

| Metric | 12-Hour Grid | Radial Dial | Mean Diff [95% CI] | Paired $t$-Test | $d_z$ | Pooled $d_s$ |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Task 1: 8:00 AM (s)** | $4.81 \pm 1.40$ | $10.61 \pm 2.86$ | $5.80$ [$4.20, 7.40$] | $t(15) = 7.71, p < 0.001^*$ | $1.93$ | $2.58$ |
| **Task 2: 1:30 PM (s)** | $6.21 \pm 1.80$ | $11.79 \pm 3.16$ | $5.58$ [$3.78, 7.39$] | $t(15) = 6.59, p < 0.001^*$ | $1.65$ | $2.17$ |
| **Task 3: 9:42 PM (s)** | $8.90 \pm 2.61$ | $13.40 \pm 3.82$ | $4.50$ [$2.38, 6.62$] | $t(15) = 4.52, p < 0.001^*$ | $1.13$ | $1.37$ |
| **Total Time (s)** | $\mathbf{19.91 \pm 4.55}$ | $\mathbf{35.79 \pm 8.27}$ | $\mathbf{15.88}$ [$\mathbf{11.41, 20.36}$] | $\mathbf{t(15) = 7.56, p < 0.001^*}$ | $\mathbf{1.89}$ | $\mathbf{2.38}$ |
| **Touch Errors** | $\mathbf{0.44 \pm 0.63}$ | $\mathbf{2.19 \pm 1.28}$ | $\mathbf{1.75}$ [$\mathbf{1.01, 2.49}$] | $\mathbf{t(15) = 5.03, p < 0.001^*}$ | $\mathbf{1.26}$ | $\mathbf{1.74}$ |
| **SEQ Score (1–7)** | $\mathbf{6.25 \pm 0.77}$ | $\mathbf{4.19 \pm 0.98}$ | $\mathbf{2.06}$ [$\mathbf{1.40, 2.72}$] | $\mathbf{t(15) = 6.67, p < 0.001^*}$ | $\mathbf{1.67}$ | $\mathbf{2.33}$ |
| **SUS Score (0–100)** | $\mathbf{83.44 \pm 9.48}$ | $\mathbf{57.19 \pm 12.45}$ | $\mathbf{26.25}$ [$\mathbf{18.24, 34.26}$] | $\mathbf{t(15) = 6.99, p < 0.001^*}$ | $\mathbf{1.75}$ | $\mathbf{2.37}$ |
| Adjective / Normative Grade | Excellent (Grade A) | Marginal (Grade D) | — | — | — | — |

*All reported p-values remain statistically significant at α = 0.05 following Holm-Bonferroni step-down correction.*

As detailed in Table 7, the proposed 12-Hour Grid Time Picker demonstrated statistically significant advantages over the radial dial across every metric ($p < 0.001$, verified post-Holm-Bonferroni correction):
- **Task Category Contrast Across Interaction Modalities:**
  - *Modal On-the-Hour Entry (Task 1: 8:00 AM):* Latency decreased by $54.7\%$ ($4.81 \pm 1.40\,\text{s}$ vs. $10.61 \pm 2.86\,\text{s}$, $t(15) = 7.71, p < 0.001, d_z = 1.93, d_s = 2.58$). This task demonstrated the largest relative efficiency gain because Dosezy's default `:00` chip affordance allowed older adults to configure the dose in exactly two stationary taps ($\text{Tap}(\text{Hour}) + \text{Tap}(\text{OK})$).
  - *Modal Quarter-Hour Entry (Task 2: 1:30 PM):* Latency decreased by $47.3\%$ ($6.21 \pm 1.80\,\text{s}$ vs. $11.79 \pm 3.16\,\text{s}$, $t(15) = 6.59, p < 0.001, d_z = 1.65, d_s = 2.17$), representing balanced discrete selection across hour and minute chips without default carryover.
  - *Non-Modal Stepper Entry (Task 3: 9:42 PM):* Latency decreased by $33.6\%$ ($8.90 \pm 2.61\,\text{s}$ vs. $13.40 \pm 3.82\,\text{s}$, $t(15) = 4.52, p < 0.001, d_z = 1.13, d_s = 1.37$). Because participants had to select chip `:45` and tap the minus stepper three times to reach 42, linear tap overhead increased setup duration, resulting in a smaller relative advantage over radial steering. This empirically illustrates the Granularity-Friction trade-off: discrete stepping eliminates trajectory steering errors at the cost of incremental tap volume.
- **44.4% Overall Reduction in Total Task Completion Time:** Aggregate completion time across all three tasks decreased from $35.79 \pm 8.27\,\text{s}$ on the radial dial to $19.91 \pm 4.55\,\text{s}$ on the 12-hour grid ($t(15) = 7.56, p < 0.001$, mean difference $15.88\,\text{s}$ [95% CI: $11.41, 20.36$], $d_z = 1.89$, pooled $d_s = 2.38$).
- **80.0% Reduction in Touch Input Errors:** Algorithmically detected touch input errors dropped from $2.19 \pm 1.28$ errors per participant on the radial dial to $0.44 \pm 0.63$ on the grid picker ($t(15) = 5.03, p < 0.001$, mean difference $1.75$ errors [95% CI: $1.01, 2.49$], $d_z = 1.26$, pooled $d_s = 1.74$). On the radial dial, participants with tremors repeatedly slipped into adjacent quadrants when attempting to release the minute hand at exact increments.
- **Significant Usability Enhancement (SUS):** Dosezy achieved a composite System Usability Scale score of **83.44 ± 9.48** (95% CI: $[78.39, 88.49]$), corresponding to the 'Excellent' / Grade A band under the normative evaluation framework of Bangor et al. [14, 15]. By contrast, the radial clock interface scored $57.19 \pm 12.45$ (95% CI: $[50.56, 63.82]$), representing marginal/failing usability (Grade D). The SUS was administered independently following each condition.

### 6.4 Exploratory Configuration and Permission-Surface Analysis (Addressing RQ4)

To explore configuration workflow characteristics and permission surface area relative to commercial mHealth software, we conducted an exploratory cognitive walkthrough comparison against Medisafe (v9.12), a representative commercial medication reminder application on Google Play.
...
#### Table 8: Exploratory Configuration and Permission-Surface Analysis: Medisafe v9.12 vs. Dosezy
...
| **Time to Configure First Dose (Researcher-Observed Walkthrough)** | $102 \pm 18\,\text{seconds}$ | $28 \pm 6\,\text{seconds}$ |
...
As summarized in Table 8, the Medisafe v9.12 onboarding flow required navigating a 9-screen wizard involving mandatory cloud sign-in prompts, promotional offers, shape/color customization pickers, and multi-step scrolling selectors, averaging $102 \pm 18\,\text{seconds}$ across 5 researcher walkthrough trials. In contrast, Dosezy's single-screen configuration dialog allowed the same regimen to be configured in $28 \pm 6\,\text{seconds}$ ($72.5\%$ reduction in setup duration), free of advertisements and background telemetry. We emphasize that this evaluation represents an exploratory expert cognitive walkthrough characterizing interface steps and permission surface area, rather than a controlled trial with older adult participants.
...
Internal validity concerns whether the observed usability improvements can be definitively attributed to the 12-Hour Grid design rather than confounding artifacts. A primary risk in within-subjects studies is task ordering and practice effects. We mitigated this by counterbalancing condition order ($n=8$ starting with Grid, $n=8$ starting with Radial) and evaluating potential sequence carryover via independent-samples $t$-tests on within-subject difference scores ($t(14) = 0.38, p = 0.71$). We did not detect evidence of a sequence effect in this sample, although with $n=8$ per sequence, statistical power to formally establish equivalence is limited.

Several design and measurement boundaries warrant discussion:
1. *Baseline Construction:* Condition B evaluated the radial dial mode of the Material 3 `TimePicker` with keyboard text input disabled to isolate continuous curved motor steering friction. While this setup directly answers RQ1 regarding radial dragging mechanics, it does not compare against numeric keypad typing or alternative discrete time pickers (e.g., vertical scroll wheels or dropdown spinners).
2. *Algorithmic Error Thresholds:* Touch input errors were operationalized using coordinate-based heuristics ($1.5\,\text{s}$ reselection and $15^\circ$ steering abort). Although algorithmic logging eliminates subjective observer scoring bias, these fixed thresholds cannot completely distinguish deliberate exploratory hesitation from involuntary motor slips.
3. *Default Affordance and Task Categories:* The substantial 54.7% latency reduction in Task 1 (8:00 AM) is partly driven by Dosezy's `:00` default minute affordance, which reduces input to two stationary taps. When participants configure non-modal times requiring incremental stepper adjustments (Task 3: 9:42 PM), the latency advantage narrows to 33.6% due to linear tap accumulation. This contrast reflects the real-world trade-off of the Modal-First principle rather than experimental bias.

### 7.2 External Validity
External validity concerns the generalizability of our findings beyond the evaluated sample. Our usability evaluation represents an initial controlled laboratory study with 16 older adults recruited from urban community senior centers in Hyderabad, India. While all participants met our inclusion criteria (aged 60+, active medication users, MMSE $\ge 27/30$), our sample may not generalize to individuals with advanced neurodegenerative impairments (e.g., late-stage Alzheimer's, severe Parkinsonian tremors), older adults in rural environments with zero previous smartphone literacy, or diverse cultural cohorts. Furthermore, all usability tasks were evaluated on a single physical smartphone model (Samsung Galaxy A13); evaluating interaction across different screen sizes, aspect ratios, and varying hand postures represents essential future work.
...
In this paper, we presented **Dosezy**, an open-source, local-first Android medication reminder system engineered specifically to resolve the physical, cognitive, and operating system barriers that impede geriatric mobile health adoption. Addressing four core research questions, our evaluation indicates under tested conditions:
1. **RQ1 (Interaction Accessibility):** In an initial controlled laboratory study of 16 older adults, replacing continuous radial clock dragging with a stationary 12-Hour Grid Time Picker reduced total task completion time by $44.4\%$ ($19.91\,\text{s}$ vs. $35.79\,\text{s}$, $t(15) = 7.56, p < 0.001$) and touch input errors by $80.0\%$ ($0.44$ vs. $2.19$, $t(15) = 5.03, p < 0.001$), yielding a mean System Usability Scale score of $83.44 \pm 9.48$ (corresponding to Grade A, 'Excellent' under the normative framework of Bangor et al. [14, 15]), while characterizing the linear tap overhead associated with off-quarter stepper adjustment.
2. **RQ2 (Alarm Reliability under Doze):** In a five-device, 100-alarm benchmark under tested deep Doze conditions, elevating reminder alerts to `setAlarmClock` paired with exact alarm permissions and compliant lock screen full-screen intents delivered 99 of 100 test alarms within 5 seconds ($99.0\%$, exact Wilson 95% CI $[94.55\%, 99.82\%]$) across five physical OEM distributions, while empirically demonstrating that application-space design cannot completely eliminate vendor-level battery killer interference without manual user whitelisting.
3. **RQ3 (Runtime Efficiency):** Local-first embedded Room SQLite persistence maintained sub-second cold starts ($< 780\,\text{ms}$) and query latencies under $12\,\text{ms}$ on budget hardware (Samsung Galaxy A13), while generating patient vector A4 summaries locally in $142 \pm 18\,\text{ms}$, confirming engineering feasibility against Android 60 fps (16.6 ms) frame budgets.
4. **RQ4 (Configuration Overhead):** In an exploratory configuration walkthrough case study, eliminating mandatory cloud registration and multi-screen onboarding wizards enabled initial dose setup in $28 \pm 6\,\text{seconds}$ ($72.5\%$ faster than the tested Medisafe workflow), while maintaining an architectural network isolation boundary through the total omission of `android.permission.INTERNET`.
...
### 8.1 Artifact and Data Availability

To support independent scientific verification and reproducible research, all empirical datasets, benchmark logs, and reproduction scripts have been deposited in a public supplementary research repository:
- **Participant-Level Usability Dataset:** [`research/supplementary/usability_participants.csv`](https://github.com/saaduddin/dosezy/blob/main/research/supplementary/usability_participants.csv) contains the complete de-identified records for all $N=16$ older-adult participants (individual task times, errors, SEQ ratings, SUS scores, and demographic variables).
- **Physical Multi-OEM Alarm Dataset:** [`research/supplementary/oem_alarm_benchmark.csv`](https://github.com/saaduddin/dosezy/blob/main/research/supplementary/oem_alarm_benchmark.csv) contains the 100 individual physical alarm trigger records across five OEM smartphone distributions under forced deep Doze mode.
- **Automated Reproduction Script:** [`research/supplementary/reproduce_paper_stats.py`](https://github.com/saaduddin/dosezy/blob/main/research/supplementary/reproduce_paper_stats.py) independently re-runs all Student's $t$-tests, confidence intervals, effect sizes, and Holm-Bonferroni corrections reported in Tables 4, 5, and 7 with a single command.
- **Application Source Code & Build Assets:** The complete Android source code, Jetpack Compose layouts, Room schemas, and build definitions are available under an open-source license at [https://github.com/saaduddin/dosezy](https://github.com/saaduddin/dosezy).
...
23. Google Android Developers, "Build an offline-first app: Architecture and local persistence," Google Developer Documentation, 2024. URL: [https://developer.android.com/topic/architecture/data-layer/offline-first](https://developer.android.com/topic/architecture/data-layer/offline-first).
24. Google Android Developers, "Schedule alarms and handle exact alarm permissions," Google Developer Documentation, 2024. URL: [https://developer.android.com/develop/background-work/services/alarms](https://developer.android.com/develop/background-work/services/alarms).
25. Google Android Developers, "Changes to full-screen intent permissions in Android 14," Google Developer Documentation, 2024. URL: [https://developer.android.com/about/versions/14/changes/fsi-rules](https://developer.android.com/about/versions/14/changes/fsi-rules).
26. A. M. Piper, R. Campbell, and J. D. Hollan, "Exploring the accessibility and appeal of surface computing for older adult health care support," in *Proceedings of the SIGCHI Conference on Human Factors in Computing Systems (CHI '10)*, ACM, New York, NY, USA, pp. 907–916, 2010. DOI: [10.1145/1753326.1753461](https://doi.org/10.1145/1753326.1753461).
