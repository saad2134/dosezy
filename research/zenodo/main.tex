...
\author{
  \textbf{Saaduddin Mohammad}\textsuperscript{1,*}, \textbf{Md Rahif Uddin Khan}\textsuperscript{1}, \textbf{Khwaja Mohammed}\textsuperscript{1} \\[0.5em]
  \small \textsuperscript{1}Department of Computer Science and Engineering, Methodist College of Engineering and Technology, Hyderabad, India \\[0.2em]
  \small \texttt{\{160723733051, 160723733005, 160723733063\}@methodist.edu.in} \\[0.2em]
  \small ORCID: \href{https://orcid.org/0009-0008-7954-7696}{0009-0008-7954-7696} (S.~Mohammad), \href{https://orcid.org/0009-0009-6219-4924}{0009-0009-6219-4924} (M.~R.~U.~Khan), \href{https://orcid.org/0009-0007-6312-6347}{0009-0007-6312-6347} (K.~Mohammed)
}
...
\begin{abstract}
Prescription non-adherence among older adults poses substantial health risks and generates preventable healthcare expenditures. Although mobile medication reminder applications are widely available, existing commercial and open-source tools exhibit fundamental human-computer interaction (HCI) and background scheduling limitations when deployed in geriatric contexts: they rely on circular radial clock dials that require fine continuous motor tracing, enforce mandatory cloud registration that aggregates sensitive medical profiles, and suffer from dropped notifications caused by aggressive Android OEM background battery optimization. While individual foundational primitives---such as Fitts' law target sizing, relational SQLite storage, \texttt{AlarmManager.setAlarmClock} scheduling, and standardized usability scales---are well established, their unified integration into a cohesive, geriatric-accessible, and offline-contained mobile architecture has not been previously investigated in medication reminder systems. We present \textbf{Dosezy}, an open-source, local-first Android medication reminder system engineered specifically to address these physical, cognitive, and operating system barriers. Dosezy introduces a stationary 12-Hour Grid Time Picker featuring a 12-first layout and oversized $\ge 56\,\text{dp}$ touch targets, enforces architectural network isolation via manifest privilege minimization through the total omission of the \texttt{android.permission.INTERNET} permission, generates patient medication and adherence summary PDFs entirely on-device, and implements a multi-stage background scheduling pipeline combining \texttt{setAlarmClock} and exact alarm APIs with full-screen lock screen intent handling. We report empirical evaluations across systems and human factors dimensions: (1) multi-device runtime profiling on flagship and entry-level hardware demonstrating cold startups under $780\,\text{ms}$, vector PDF generation in $142 \pm 18\,\text{ms}$, and database query latencies under $12\,\text{ms}$, establishing engineering feasibility against Android 60~fps (16.6~ms) frame budgets; (2) a multi-OEM Doze mode alarm benchmark across five physical devices (99 of 100 scheduled alerts met the prespecified $\le 5.0\,\text{s}$ criterion under tested deep Doze and device conditions, 99.0\%, 95\% CI $[94.55\%, 99.82\%]$), confirming that application-level techniques cannot completely eliminate OEM-specific power-management interference without user-side whitelisting; and (3) an initial counterbalanced within-subjects laboratory usability study ($N=16$ older adults, aged 61--76, mean age $68.5 \pm 4.8$ years) demonstrating a $44.4\%$ reduction in total time-setting latency ($19.9\,\text{s}$ vs.\ $35.8\,\text{s}$, $p < 0.001$), an $80.0\%$ reduction in touch input errors ($0.44$ vs.\ $2.19$, $p < 0.001$), and a System Usability Scale (SUS) score of $83.44 \pm 9.48$ (corresponding to Grade A, `Excellent' under the normative framework of Bangor et al.~\cite{bangor2008empirical,lewis2009factor}) compared to $57.19 \pm 12.45$ for standard radial dials. We frame our contributions as systems and usability engineering prerequisites for medication adherence, discussing threats to validity and longitudinal trade-offs.
\end{abstract}
...
Addressing these research questions motivated the design, implementation, and empirical evaluation of \textbf{Dosezy}, an open-source, local-first Android medication reminder platform. While foundational interaction and systems primitives---including Fitts' law target scaling, relational SQLite storage, \texttt{AlarmManager.setAlarmClock} scheduling, and standardized usability metrics---are established, to the best of our knowledge, prior work has not empirically evaluated this specific systems integration of discrete stationary time input, local-only data containment, and Doze-oriented alarm scheduling in a digital medication reminder system designed specifically for older adults. Our research makes four primary contributions:

\begin{itemize}[leftmargin=*]
    \item \textbf{Integrated Accessibility-Centric Interaction Architecture:} We propose and validate the \emph{12-Hour Grid Time Picker}, an interface replacing circular dragging gestures with a 12-first discrete button grid ($56\times56\,\text{dp}$ touch targets) and quick-selection minute chips (:00, :15, :30, :45) that enable time configuration in as few as two stationary taps, paired with a visual palette whose primary text/surface pair achieves a measured contrast ratio of $7.84:1$, exceeding the WCAG 2.1 AAA minimum threshold ($7.0:1$) for normal text~\cite{w3c2018wcag}.
    \item \textbf{Architectural Network Isolation via Privilege Minimization:} We present a local-first systems architecture that enforces on-device data containment by completely omitting \texttt{android.permission.INTERNET} from the manifest. This establishes an operating-system-level socket denial boundary that prevents ordinary application-level network access through Android's permission-controlled socket APIs, coupled with an on-device vector A4 PDF export engine ($142 \pm 18\,\text{ms}$) that executes without remote cloud dependencies~\cite{kleppmann2019local}.
    \item \textbf{Doze-Resilient Alarm Scheduling Pipeline:} We engineer a multi-stage background alarm dispatch pipeline combining \texttt{AlarmManager.setAlarmClock}, exact alarm privileges, and full-screen lock screen intents to maximize timely dosage delivery during deep Doze states across fragmented Android OEM distributions, while characterizing the OS-level boundaries of vendor-specific background optimization.
    \item \textbf{Empirical Systems and Usability Validation:} We conduct hardware runtime profiling establishing sub-frame execution feasibility, benchmark background alarm delivery across five smartphone OEMs under deep Doze mode ($99.0\%$ on-time delivery across 100 test alarms under tested conditions), and execute an initial counterbalanced within-subjects laboratory usability study ($N=16$ older adults, aged 61--76) demonstrating statistically significant reductions in task completion time ($44.4\%$) and input errors ($80.0\%$), yielding a mean System Usability Scale (SUS) score of $83.44 \pm 9.48$, corresponding to Grade A (`Excellent') under the normative framework of Bangor et al.~\cite{bangor2008empirical,lewis2009factor} compared to $57.19 \pm 12.45$ for standard radial dials.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Commercial Mobile Applications:} Commercial mHealth apps (e.g., Medisafe, MyTherapy) operate on commodity hardware already carried by patients. However, their systems architecture typically introduces friction for older adults. As summarized in Table~\ref{tab:comparison_matrix}, commercial applications frequently mandate remote account registration, integrate commercial advertisement banners and third-party telemetry SDKs, require navigating multi-screen onboarding wizards, and rely on standard cloud push notifications or inexact alarm primitives that are susceptible to deferral or termination when modern OEM power-management daemons force deep sleep~\cite{wildenbos2018aging,google2024offline}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Primary Tier (Modal Interval Selection):} Oversized stationary discrete targets for common quarter-hour prescription intervals (e.g., :00, :15, :30, :45) aligned with standard outpatient dosing routines, bounding motor complexity to 2--3 stationary taps.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Preset Minute Quick-Chips with Fine Adjustment Stepper:} Outpatient pharmacotherapy regimens are predominantly scheduled at standard hour and quarter-hour milestones tied to daily physiological events (e.g., waking at 8:00 AM, post-lunch at 1:30 PM, bedtime at 9:00 PM)~\cite{ada2023standards}. Dosezy provides four discrete quick-select chips: \texttt{:00}, \texttt{:15}, \texttt{:30}, and \texttt{:45}. Because the interface pre-selects the \texttt{:00} minute chip and defaults to the current diurnal period (e.g., AM for morning sessions), scheduling an on-the-hour dose requires exactly two taps:
    \begin{equation}
...
\end{equation}
This measured value exceeds the $7.0:1$ contrast ratio mandated by WCAG 2.1 Level AAA for normal text~\cite{w3c2018wcag}, specifically ensuring that the primary typography and key visual containers maintain high contrast under suboptimal viewing angles and degraded retinal illumination.
...
\begin{algorithm}[t]
\caption{Alarm Scheduling and Doze-Resilient Dispatch Pipeline}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{System Alarm Clock Elevation (\texttt{setAlarmClock}):} Rather than using standard idle-tolerant alarms, Dosezy registers reminders via \texttt{AlarmManager.setAlarmClock(AlarmClockInfo, PendingIntent)}. The Android operating system treats \texttt{setAlarmClock} as a high-priority alarm type under the Android Open Source Project (AOSP), originally reserved for user-facing clock utilities. Alarms scheduled through this method:
    \begin{itemize}
        \item Cause the operating system to exit low-power idle modes (Doze) to deliver the alarm broadcast precisely at the scheduled real-time clock (\texttt{RTC\_WAKEUP}) tick.
        \item Cause the system status bar to display the official platform alarm clock icon, providing visual confirmation that an alert is armed.
        \item Are exempted from standard background power throttling, although they do not bypass Android 10+ background activity launch restrictions---which is precisely why Dosezy pairs alarm reception with full-screen notification intents.
    \end{itemize}
    ...
    \end{itemize}
    For distribution targeting API 33+, Dosezy declares \texttt{USE\_EXACT\_ALARM} under the core alarm exemption, with fallback to \texttt{SCHEDULE\_EXACT\_ALARM} qualified for API 31--32 via \texttt{android:maxSdkVersion="32"} or runtime status checking via \texttt{AlarmManager.canScheduleExactAlarms()}. At runtime, the application evaluates \texttt{AlarmManager.canScheduleExactAlarms()}. If exact scheduling is revoked or denied (such as on fresh Android 14 installations where users or OEM policies have revoked exact alarm capability), Dosezy presents a high-contrast explanation dialog and dispatches an explicit system intent:
    \begin{equation*}
    ...
    \item \textbf{Full-Screen Lock Screen Activity Overlay via Compliant Intents:} Under Android 10+ Background Activity Launch rules, background receivers cannot directly call \texttt{startActivity()}. Dosezy conforms by dispatching a high-priority heads-up notification with \texttt{setFullScreenIntent(pendingIntent, true)}. The target \texttt{AlarmActivity} configures its window via API 27+ methods \texttt{setShowWhenLocked(true)} and \texttt{setTurnScreenOn(true)}, paired with \texttt{FLAG\_KEEP\_SCREEN\_ON}. Starting in Android 14 (API 34), Google restricted the automatic grant of \texttt{android.permission.USE\_FULL\_SCREEN\_INTENT} to applications primarily categorized as calling or alarm utilities on Google Play~\cite{google2024fsi}. Applications submitting to commercial stores outside these categories have the permission revoked by default. Dosezy handles this operating system boundary through runtime verification: on API 34+, the application evaluates \texttt{NotificationManager.canUseFullScreenIntent()}. If granted (as is standard in alarm-category listings and open-source distributions such as F-Droid), the system immediately illuminates the screen and presents the full-screen dosage prompt over the keyguard. If revoked, the notification gracefully falls back to an expanded heads-up banner in the active notification shade with audible and tactile alerting, while prompting the user to grant full-screen access via \texttt{Settings.ACTION\_MANAGE\_APP\_USE\_FULL\_SCREEN\_INTENT}. When active, the overlay enables older patients to visually review the medication name, dosage strength, and instructions, tapping ``TAKEN'' or ``SNOOZE 10M'' with a single oversized tap without navigating lock screen PINs, patterns, or biometric sensors.
...
\emph{Note: The offline emergency dispatch resolution table is provided as an immediate convenience feature and is not a substitute for authoritative local emergency-service information.}
...
\end{equation}
An alarm was operationalized as \textbf{ON-TIME} if $\Delta t_{\text{latency}} \le 5.0\,\text{seconds}$. We operationalized on-time delivery as $\le 5.0\,\text{seconds}$ because medication reminders are immediate, user-facing critical alerts where a perceptible delay is disruptive, whereas general background data synchronization tolerances can extend over minutes or hours.
...
\toprule
\textbf{Device Model} & \textbf{OS Distribution} & \textbf{On-Time Delivery} & \textbf{Mean Latency (s)} & \textbf{Max Latency (s)} & \textbf{Success Rate (95\% CI)} \\
\midrule
...
\midrule
\textbf{Total / Aggregate} & --- & \textbf{99 / 100} & \textbf{2.57 $\pm$ 6.66 (incl.); 1.91 $\pm$ 0.44 (excl.)} & \textbf{68.4 (incl.); 2.9 (excl.)} & \textbf{99.0\% [94.55\%, 99.82\%]} \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^*$Note on Outlier Dispersion: Across the 19 on-time alarms on Xiaomi, mean latency was $1.52 \pm 0.28\,\text{s}$ (max $2.0\,\text{s}$).} \\
\multicolumn{6}{l}{\footnotesize Including the single $68.4\,\text{s}$ powerkeeper delay expands sample SD to $14.96\,\text{s}$ (mean $4.87\,\text{s}$). Wilson score 95\% CI: $[94.55\%, 99.82\%]$.}
\end{tabular}
\end{table}

\emph{Benchmark Delivery Analysis and OEM Throttling Observations:} In our five-device, 100-alarm benchmark under tested deep Doze conditions, 99 of 100 alarms ($99.0\%$, 95\% CI $[94.55\%, 99.82\%]$) triggered within 5 seconds (Table~\ref{tab:oem_benchmark}). Crucially, this empirical benchmark illustrates that application-space design cannot completely eliminate vendor-level battery killer interference across all hardware without explicit user whitelisting. The sole observed delay occurred on the Xiaomi Redmi Note 8T running MIUI 14, where a single alarm was held back for $68.4\,\text{seconds}$ until the screen was illuminated. Telemetry inspection confirmed that Xiaomi's proprietary powerkeeper daemon (\texttt{com.miui.powerkeeper}) temporarily suspended broadcast execution despite the elevated \texttt{setAlarmClock} status. A similar delay profile occurs on Samsung One UI if an app is categorized under ``Deep Sleeping Apps''. While \texttt{setAlarmClock} provides the highest execution priority granted by AOSP, vendor-specific battery daemons remain an operating system barrier that cannot be unilaterally bypassed without user-side battery exemption. When users manually exempt Dosezy from background restrictions via system settings, delivery across test trials reached 100\% ($20/20$).
...
\paragraph{Timing Instrumentation \& Algorithmic Error Operationalization:} Interaction timestamps were captured automatically with millisecond precision via Jetpack Compose \texttt{pointerInput} event listeners attached to the root dialog surface. Touch input errors ($TIE$) were operationalized as algorithmically detected touch-error events through deterministic coordinate logging logic:
\begin{itemize}[leftmargin=*]
    \item \emph{Target Slip (Grid \& Radial):} Recorded when pointer-down coordinates fell within target bounding box $B_i$, but pointer-up coordinates fell outside $B_i$ or crossed gutter boundaries prior to release ($\text{target}_{\text{down}} \ne \text{target}_{\text{up}}$).
    \item \emph{Immediate Reselection (Grid):} Recorded when an hour or minute chip was selected and subsequently replaced by an adjacent target within $1.5\,\text{seconds}$ without an intervening confirmation tap. The 1.5-second threshold operationalizes Fitts' hesitation and recovery model, capturing instances where an older adult corrects an unintended adjacent tap without executing an intermediate action.
    \item \emph{Quadrant Steering Abort (Radial):} Recorded when a continuous circular drag in Condition B traversed through an incorrect angular quadrant and was released at an angular deviation $|\theta_{\text{release}} - \theta_{\text{target}}| > 15^\circ$, necessitating an immediate secondary correction drag. The 15° threshold corresponds to half the angular width of an hour sector ($360^\circ / 12 = 30^\circ$), capturing instances where continuous steering crossed the midpoint into an adjacent sector, requiring trajectory realignment.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Condition B (Radial Dial Mode Baseline):} Setting reminder times using the official Material Design 3 Jetpack Compose \texttt{TimePicker} (\texttt{androidx.compose.material3:material3:1.2.0}, vertical layout \texttt{TimePickerLayoutType.Vertical}) running in 12-hour radial dial mode. To isolate pure continuous circular motor trajectory steering friction, the alternative soft-keyboard text input toggle was disabled. We explicitly acknowledge this methodological boundary: while standard Android deployments permit toggling between radial dragging and text entry, disabling the toggle isolated curved motor steering mechanics from numeric soft-keyboard typing, while framing keyboard input and other discrete pickers as important targets for future comparative evaluation.
\end{itemize}
...
\midrule
\textbf{Task 1: 8:00 AM (s)} & $4.81 \pm 1.40$ & $10.61 \pm 2.86$ & $5.80\ [4.20, 7.40]$ & $t(15) = 7.71, p < 0.001^*$ & $1.93$ & $2.58$ \\
\textbf{Task 2: 1:30 PM (s)} & $6.21 \pm 1.80$ & $11.79 \pm 3.16$ & $5.58\ [3.78, 7.39]$ & $t(15) = 6.59, p < 0.001^*$ & $1.65$ & $2.17$ \\
\textbf{Task 3: 9:42 PM (s)} & $8.90 \pm 2.61$ & $13.40 \pm 3.82$ & $4.50\ [2.38, 6.62]$ & $t(15) = 4.52, p < 0.001^*$ & $1.13$ & $1.37$ \\
\midrule
\textbf{Total Time (s)} & $\mathbf{19.91 \pm 4.55}$ & $\mathbf{35.79 \pm 8.27}$ & $\mathbf{15.88\ [11.41, 20.36]}$ & $\mathbf{t(15) = 7.56, p < 0.001^*}$ & $\mathbf{1.89}$ & $\mathbf{2.38}$ \\
\textbf{Touch Errors} & $\mathbf{0.44 \pm 0.63}$ & $\mathbf{2.19 \pm 1.28}$ & $\mathbf{1.75\ [1.01, 2.49]}$ & $\mathbf{t(15) = 5.03, p < 0.001^*}$ & $\mathbf{1.26}$ & $\mathbf{1.74}$ \\
\textbf{SEQ Score (1--7)} & $\mathbf{6.25 \pm 0.77}$ & $\mathbf{4.19 \pm 0.98}$ & $\mathbf{2.06\ [1.40, 2.72]}$ & $\mathbf{t(15) = 6.67, p < 0.001^*}$ & $\mathbf{1.67}$ & $\mathbf{2.33}$ \\
\midrule
\textbf{SUS Score (0--100)} & $\mathbf{83.44 \pm 9.48}$ & $\mathbf{57.19 \pm 12.45}$ & $\mathbf{26.25\ [18.24, 34.26]}$ & $\mathbf{t(15) = 6.99, p < 0.001^*}$ & $\mathbf{1.75}$ & $\mathbf{2.37}$ \\
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Task Category Contrast Across Interaction Modalities:}
    \begin{itemize}
        \item \emph{Modal On-the-Hour Entry (Task 1: 8:00 AM):} Latency decreased by $54.7\%$ ($4.81 \pm 1.40\,\text{s}$ vs.\ $10.61 \pm 2.86\,\text{s}$, $t(15) = 7.71, p < 0.001, d_z = 1.93, d_s = 2.58$). This task demonstrated the largest relative efficiency gain because Dosezy's default :00 chip affordance allowed older adults to configure the dose in exactly two stationary taps ($\text{Tap}(\text{Hour}) + \text{Tap}(\text{OK})$).
        \item \emph{Modal Quarter-Hour Entry (Task 2: 1:30 PM):} Latency decreased by $47.3\%$ ($6.21 \pm 1.80\,\text{s}$ vs.\ $11.79 \pm 3.16\,\text{s}$, $t(15) = 6.59, p < 0.001, d_z = 1.65, d_s = 2.17$), representing balanced discrete selection across hour and minute chips without default carryover.
        \item \emph{Non-Modal Stepper Entry (Task 3: 9:42 PM):} Latency decreased by $33.6\%$ ($8.90 \pm 2.61\,\text{s}$ vs.\ $13.40 \pm 3.82\,\text{s}$, $t(15) = 4.52, p < 0.001, d_z = 1.13, d_s = 1.37$). Because participants had to select chip \texttt{:45} and tap the minus stepper three times to reach 42, linear tap overhead increased setup duration, resulting in a smaller relative advantage over radial steering. This empirically illustrates the Granularity-Friction trade-off: discrete stepping eliminates trajectory steering errors at the cost of incremental tap volume.
    \end{itemize}
    \item \textbf{44.4\% Overall Reduction in Total Task Completion Time:} Aggregate completion time across all three tasks decreased from $35.79 \pm 8.27\,\text{s}$ on the radial dial to $19.91 \pm 4.55\,\text{s}$ on the 12-hour grid ($t(15) = 7.56, p < 0.001$, mean difference $15.88\,\text{s}$ [95\% CI: $11.41, 20.36$], $d_z = 1.89$, pooled $d_s = 2.38$).
    \item \textbf{80.0\% Reduction in Touch Input Errors:} Algorithmically detected touch input errors dropped from $2.19 \pm 1.28$ errors per participant on the radial dial to $0.44 \pm 0.63$ on the grid picker ($t(15) = 5.03, p < 0.001$, mean difference $1.75$ errors [95\% CI: $1.01, 2.49$], $d_z = 1.26$, pooled $d_s = 1.74$). On the radial dial, participants with tremors repeatedly slipped into adjacent quadrants when attempting to release the minute hand at exact increments.
    \item \textbf{Significant Usability Enhancement (SUS):} Dosezy achieved a composite System Usability Scale score of \textbf{83.44 $\pm$ 9.48} (95\% CI: $[78.39, 88.49]$), corresponding to the `Excellent' / Grade A band under the normative evaluation framework of Bangor et al.~\cite{bangor2008empirical,lewis2009factor}. By contrast, the radial clock interface scored $57.19 \pm 12.45$ (95\% CI: $[50.56, 63.82]$), representing marginal/failing usability (Grade D). The SUS was administered independently following each condition.
\end{itemize}

\subsection{Exploratory Configuration and Permission-Surface Analysis (Addressing RQ4)}
...
In this paper, we presented \textbf{Dosezy}, an open-source, local-first Android medication reminder system engineered specifically to resolve the physical, cognitive, and operating system barriers that impede geriatric mobile health adoption. Addressing four core research questions, our evaluation indicates under tested conditions:
\begin{enumerate}
    \item \textbf{RQ1 (Interaction Accessibility):} In an initial controlled laboratory study of 16 older adults, replacing continuous radial clock dragging with a stationary 12-Hour Grid Time Picker reduced total task completion time by $44.4\%$ ($19.91\,\text{s}$ vs.\ $35.79\,\text{s}$, $t(15) = 7.56, p < 0.001$) and touch input errors by $80.0\%$ ($0.44$ vs.\ $2.19$, $t(15) = 5.03, p < 0.001$), yielding a mean System Usability Scale score of $83.44 \pm 9.48$ (corresponding to Grade A, `Excellent' under the normative framework of Bangor et al.~\cite{bangor2008empirical,lewis2009factor}), while characterizing the linear tap overhead associated with off-quarter stepper adjustment.
    \item \textbf{RQ2 (Alarm Reliability under Doze):} In a five-device, 100-alarm benchmark under tested deep Doze conditions, elevating reminder alerts to \texttt{setAlarmClock} paired with exact alarm permissions and compliant lock screen full-screen intents delivered 99 of 100 test alarms within 5 seconds ($99.0\%$, exact Wilson 95\% CI $[94.55\%, 99.82\%]$) across five physical OEM distributions, while empirically demonstrating that application-space design cannot completely eliminate vendor-level battery killer interference without manual user whitelisting.
    \item \textbf{RQ3 (Runtime Efficiency):} Local-first embedded Room SQLite persistence maintained sub-second cold starts ($< 780\,\text{ms}$) and query latencies under $12\,\text{ms}$ on budget hardware (Samsung Galaxy A13), while generating patient vector A4 summaries locally in $142 \pm 18\,\text{ms}$, confirming engineering feasibility against Android 60~fps (16.6~ms) frame budgets.
...
\subsection{Artifact and Data Availability}
\label{sec:artifacts}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Participant-Level Usability Dataset:} \texttt{research/supplementary/usability\_participants.csv} contains the complete de-identified records for all $N=16$ older-adult participants (individual task times, errors, SEQ ratings, SUS scores, and demographic variables).
    \item \textbf{Physical Multi-OEM Alarm Dataset:} \texttt{research/supplementary/oem\_alarm\_benchmark.csv} contains the 100 individual physical alarm trigger records across five OEM smartphone distributions under forced deep Doze mode.
    \item \textbf{Automated Reproduction Script:} \texttt{research/supplementary/reproduce\_paper\_stats.py} independently re-runs all Student's $t$-tests, confidence intervals, effect sizes, and Holm-Bonferroni corrections reported in Tables~4, 5, and 7 with a single command.
    \item \textbf{Application Source Code \& Build Assets:} The complete Android source code, Jetpack Compose layouts, Room schemas, and build definitions are available under an open-source license at \url{https://github.com/saaduddin/dosezy}.
\end{itemize}
